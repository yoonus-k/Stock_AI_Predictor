...
class MultiThreadedParameterTester(ParameterTester):
    ...
    # Continue with the rest of the method...
        ...
        for i, cluster_center in enumerate(pip_miner._cluster_centers):
            ...
            cluster_id = i   # Use 1-based indexing for clarity
              # Calculate expected value: (probability * RR) - (1 - probability)
            expected_value = (probability_score_dir * reward_risk_ratio) - (1 - probability_score_dir)
    ...
    def calculate_metrics_gpu(self, outcomes, pattern_indices, cluster_centers, max_gain_values, max_drawdown_values):
        ...
        try:
            ...
            if avg_outcome > 0:
                ...
                max_drawdown = tf.reduce_mean(max_gain_tf).numpy()  # For sell, max drawdown is from max_gain
            else:
                label = 'Neutral'
                max_gain = 0.0
                max_drawdown = 0.0
                    ...
                    cursor.execute(
                        """INSERT INTO clusters 
                           (cluster_id,stock_id, timeframe_id, config_id, avg_price_points_json, avg_volume,
                            ...
                            max_gain, max_drawdown, reward_risk_ratio, profit_factor, expected_value, description, created_at)
                           VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                        (cluster_data['cluster_id'],cluster_data['stock_id'], cluster_data['timeframe_id'], cluster_data['config_id'],
                         ...
                         cluster_data['profit_factor'], cluster_data['expected_value'], cluster_data['description'],
    ...
    def calculate_weighted_score(self, metrics):
        ...
        # Calculate expected value for each metric if not already present
        for metric in metrics:
            if 'expected_value' not in metric:
                # Calculate expected value: (probability * RR) - (1 - probability)
                metric['expected_value'] = (metric['probability_score'] * metric['reward_risk_ratio']) - (1 - metric['probability_score'])
        
        # Normalize expected values (higher is better)
        max_expected_value = max(max([m['expected_value'] for m in metrics]), 1.0)
        normalized_expected_values = [min(1.0, max(0, m['expected_value'] / max_expected_value)) for m in metrics]
        ...
        for i, metric in enumerate(metrics):
            ...
            score = (
                ...
                weights['expected_value'] * normalized_expected_values[i]
            )
