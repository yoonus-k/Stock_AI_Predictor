"""
Backtesting Module with Cluster Pattern Matching

This module implements a backtesting framework for trading strategies
based on pattern recognition and cluster matching techniques.
It simulates trading on historical price data using patterns identified
by perceptually important points (PIPs).
"""
...
def backtest_with_cluster_matching(
    ...
    lookback=24, hold_period=6, n_pips=5 , mse_threshold=0.03
):
    """
    Perform backtesting using cluster pattern matching technique.
    
    Args:
        db: Database connection
        stock_id: ID of the stock to backtest
        ...
        mse_threshold: Threshold for mean squared error in pattern matching
        
    Returns:
        Dictionary containing performance metrics and trade details
    """
    # === Load and split data ===
    ...
    rf.fit(cluster_features, labels)

    # === Backtesting Loop ===
    returns = np.zeros(len(test_df))
    equity = [1]  # Start with 1 unit
    ...
    # plot the histogram of buy and sell trades, also the TP and SL hits
    plt.figure(figsize=(14, 8))
    plt.hist(trades_df[trades_df['type'] == 'BUY']['return_pct'], bins=20, alpha=0.5, label='Buy Trades')
    plt.hist(trades_df[trades_df['type'] == 'SELL']['return_pct'], bins=20, alpha=0.5, label='Sell Trades')
    plt.hist(trades_df[trades_df['reason'] == 'TP Hit']['return_pct'], bins=20, alpha=0.5, label='TP Hits')
    plt.hist(trades_df[trades_df['reason'] == 'SL Hit']['return_pct'], bins=20, alpha=0.5, label='SL Hits')
...
def extract_pips(window, n_pips=5, dist_type=3):
    ...
    Returns:
        tuple: (x indices, normalized y values) or None if extraction fails
    """
    try:
