...
project_root = Path(__file__).resolve().parent.parent.parent  # Points to Stock_AI_Predictor/
...
def add_technical_indicators(df):
    ...
    if len(df) >= 24:
        df['atr'] = talib.ATR(df['high_price'], df['low_price'], df['close_price'], timeperiod=24)
...
def add_time_based_features(df):
    ...
    df['asian_session'] = ((datetime_col.hour >= 0) & (datetime_col.hour < 8)).astype(float)
    df['london_session'] = ((datetime_col.hour >= 8) & (datetime_col.hour < 16)).astype(float)
    df['ny_session'] = ((datetime_col.hour >= 13) & (datetime_col.hour < 21)).astype(float)
...
def process_cot_data(cot_df, price_df):
    ...
    """
    if cot_df.empty:
        # If COT data is unavailable, add zero-filled columns
        cot_feature_names = [
            'net_noncommercial',
            'net_nonreportable',
            'change_nonrept_long',
    ...
    # Create a mapping for standardized column names
    col_mapping = {
        'net_noncommercial': ['net_noncommercial', 'netnoncommercial', 'net_noncommercial_positions'],
        'net_nonreportable': ['net_nonreportable', 'netnonreportable', 'net_nonreportable_positions'],
        'change_nonrept_long': ['change_nonrept_long', 'changenonreptlong', 'change_nonreportable_long'],
        'change_nonrept_short': ['change_nonrept_short', 'changenonreptshort', 'change_nonreportable_short'],
        'change_noncommercial_long': ['change_noncommercial_long', 'changenoncommerciallong'],
        'change_noncommercial_short': ['change_noncommercial_short', 'changenoncommercialshort']
    }
    
    # Map to standardized column names
    renamed_cols = {}
    for new_name, possible_names in col_mapping.items():
        for old_name in possible_names:
            if old_name in cot_df.columns:
                renamed_cols[old_name] = new_name
                break
    
    if renamed_cols:
        cot_df = cot_df.rename(columns=renamed_cols)
    
    # List of COT features we want to include
    cot_feature_names = list(col_mapping.keys())
...
def process_sentiment_data(sentiment_df, price_df):
    ...
    if sentiment_df.empty:
        # If sentiment data is unavailable, add zero-filled column
        price_df['unified_sentiment'] = 0.0
        return price_df
    ...
    for date in merged_df.index:
        ...
        if date_str in sentiment_df.index:
            ...
            #print(merged_df.loc[date, 'unified_sentiment'])
            sentiment_count=sentiment_df.loc[date_str, 'count']
            merged_df.at[date, 'sentiment_count'] = sentiment_count
            ...
            available_dates = sentiment_df.index[(sentiment_df.index <= date_str) & 
                                               (sentiment_df.index >= window_start)]
            
            if len(available_dates) > 0:
                # Use the most recent sentiment within the window
                most_recent = available_dates[-1]
                merged_df.at[date, 'unified_sentiment'] = sentiment_df.loc[most_recent, 'normalized']
                merged_df.at[date, 'sentiment_count'] = sentiment_df.loc[most_recent, 'count']
...
def preprocess_data(stock_data):
    ...
    sentiment_data = load_sentiment_data(symbol='XAUUSD')
    stock_data = process_sentiment_data(sentiment_data, stock_data)
    
    cot_data = fetch_cot_data(symbol='XAUUSD', start_date='2024-01-01', end_date='2026-01-01')
