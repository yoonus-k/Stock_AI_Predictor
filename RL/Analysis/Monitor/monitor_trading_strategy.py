...
class PortfolioTracker:
    ...
    def update(self, timestamp, price, position, action, reward):
        ...
        self.positions.append(
            {"timestamp": timestamp, "position": position, "price": price}
        )
        ...
        # Check for closed trade
            ...
            if self.current_trade is not None:
                ...
                if old_position > 0:  # Long position
                    profit_loss = (price - self.current_trade["entry_price"]) * abs(
                        old_position
                    )
                else:  # Short position
                    profit_loss = (self.current_trade["entry_price"] - price) * abs(
                        old_position
                    )
                ...
                if old_position > 0:  # Long position
                    self.current_trade["return_pct"] = (
                        (price / self.current_trade["entry_price"]) - 1
                    ) * 100
                else:  # Short position
                    self.current_trade["return_pct"] = (
                        (self.current_trade["entry_price"] / price) - 1
                    ) * 100

                # Calculate duration (assuming timestamps are datetime objects)
                if isinstance(timestamp, (pd.Timestamp, datetime)):
                    duration = (
                        timestamp - self.current_trade["entry_time"]
    ...
    def get_performance_metrics(self):
        ...
        metrics["long_win_rate"] = (
            (
                len([t for t in long_trades if t["profit_loss"] > 0])
                / len(long_trades)
                * 100
            )
            if long_trades
        ...
        metrics["short_win_rate"] = (
            (
                len([t for t in short_trades if t["profit_loss"] > 0])
                / len(short_trades)
                * 100
            )
            if short_trades
...
def run_model_on_data(model, env, data=None, max_steps=None, deterministic=False):
    ...
    all_data = (
        pd.concat(episodes_data, ignore_index=True) if episodes_data else pd.DataFrame()
    )
...
def plot_portfolio_performance(tracker, price_data=None, save_path=None):
    ...
    ax2.plot(
        timestamps, tracker.portfolio_values, label="Portfolio Value", color="blue"
    )
...
def plot_action_distribution(actions_data, save_path=None):
    """
    Plot distribution of trading actions for Box action spaces
    ...
    # First, convert Box actions to discrete action types
    # If actions are arrays (from Box space), extract the first element and discretize
    discrete_actions = []

    # Check if we're dealing with Box actions
    sample_action = actions_data["action"].iloc[0] if not actions_data.empty else None

    if isinstance(sample_action, (list, np.ndarray)) or (
        hasattr(sample_action, "__len__") and not isinstance(sample_action, str)
    ):        # MultiDiscrete actions - extract discrete action type directly
        for action in actions_data["action"]:
            # Extract first element (action type) - already a discrete value 0, 1, or 2
            discrete_action = int(action[0])  # 0=HOLD, 1=BUY, 2=SELL
            discrete_actions.append(discrete_action)
    else:
        # Already discrete actions
        discrete_actions = actions_data["action"]

    # Count actions by type using the discrete mapped values
    action_counts = pd.Series(discrete_actions).value_counts()
    
    # Create a normalized dictionary to ensure we have exactly 3 entries
    # with the keys 0, 1, 2 corresponding to Hold, Buy, Sell
    action_dict = {0: 0, 1: 0, 2: 0}  # Initialize with zeros
    
    # Update with actual counts
    for idx, count in action_counts.items():
        if idx in action_dict:
            action_dict[idx] = count
    
    # Create ordered arrays for plotting
    action_labels = ["Hold", "Buy", "Sell"]
    count_values = [action_dict[0], action_dict[1], action_dict[2]]
    bar_colors = ["gray", "green", "red"]

    # Plot action distribution
    axs[0, 0].bar(action_labels, count_values, color=bar_colors)
...
def analyze_trading_strategy(model_path, data=None, output_dir=None, episodes=1):
    ...
    plot_portfolio_performance(
        tracker, save_path=output_dir / "portfolio_performance.png"
    )
    ...
    plot_action_distribution(
        actions_df, save_path=output_dir / "action_distribution.png"
    )

    # Analyze trades
    print("\nAnalyzing trades...")
    plot_trade_analysis(tracker, save_path=output_dir / "trade_analysis.png")
    ...
    portfolio_df = pd.DataFrame(
        {"timestamp": tracker.timestamps, "portfolio_value": tracker.portfolio_values}
    )
    ...
    return {"tracker": tracker, "actions_data": actions_df, "metrics": metrics}


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyze trading strategy of a trained RL model"
    )
    parser.add_argument("--model",default="RL/Models/Experiments/best_model_continued.zip", type=str, help="Path to the trained model")
    parser.add_argument(
        "--output-dir",
        type=str,
        default="Images/RL/Strategy",
        help="Directory to save analysis results",
    )
    parser.add_argument(
        "--episodes", type=int, default=1, help="Number of episodes for evaluation"
    )
