...
class TradingEnvV2(gym.Env):
    ...
    def __init__(
        ...
        enable_short=True,
        margin_requirement=0.01,  # 10% margin requirement by default
        max_leverage=100,  # 10x max leverage by default
        verbose=False
    ):
        """
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.features is not None:
            self.latest_features_timestamp = self.features.iloc[0].name
            self.latest_features_data = self.latest_features_data = self._features_array[0]
