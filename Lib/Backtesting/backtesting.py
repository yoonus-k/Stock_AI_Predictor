...
class Strategy(metaclass=ABCMeta):
    ...
    def buy(self, *,
        ...
        return self._broker.new_order(size, limit, stop, sl, tp, tag,hold_time)
    ...
    def sell(self, *,
             ...
             tp: Optional[float] = None,
             tag: object = None,
             hold_time: Optional[float]) -> 'Order':
        """
    ...
    @property
    def exit_reasons_counts(self) -> dict:
        """
        Count of exit reasons for closed trades.
        Returns a dictionary with keys 'tp', 'sl', 'time' and their respective counts.
        """
        return self._broker.exit_reasons_counts
        
    @property
    def win_rate(self) -> float:
        """
        Win rate of closed trades, i.e. the ratio of profitable trades
        to total number of closed trades.
        """
        if self._broker.trade_count == 0:
            return 0.5 # If no trades, return 50% win rate by default
        return self._broker.wining_trade_count / self._broker.trade_count
...
class Order:
    ...
    def hold_time(self) -> Optional[float]:
        """
        The number of time in hours the order will be held.
        This is calculated as the difference between the current bar index and the entry bar index.
        """
...
class Trade:
    ...
    """
    def __init__(self, broker: '_Broker', size: int, entry_price: float, entry_bar, tag , hold_time: Optional[float] = None):
        self.__broker = broker
        self.__size = size
        ...
        # Add new attributes to store the price values independently
        self.__sl_price: Optional[float] = None
        self.__tp_price: Optional[float] = None
        self.__hold_time: Optional[int] = hold_time # Number of time in hours the trade will be held
        self.__exit_reason: Optional[str] = None
    ...
    def exit_reason(self) -> Optional[str]:
        """
        The reason for the trade exit, if it has been closed.
        Possible values are 'tp', 'sl', 'time', or None if the trade is still active.
        """
    ...
    def hold_time(self) -> Optional[int]:
        """
        The number of time in hours the trade will be held.
        This is calculated as the difference between the exit and entry bar indices,
        """
        return self.__hold_time
    ...
    def sl_price(self):
        """
        The price value of the stop-loss order.
        This is a separate property to store the price persistently.
        """
        return self.__sl_price
    ...
    def sl(self, price: float):
        # Store the price value persistently
        self.__sl_price = price
    ...
    def tp_price(self):
        """
        The price value of the take-profit order.
        This is a separate property to store the price persistently.
        """
        return self.__tp_price
    @tp.setter
    ...
    @exit_reason.setter
    def exit_reason(self, reason: str):
        """
        Set the reason for the trade exit.
        Possible values are 'tp', 'sl', 'time', or None if the trade is still active.
        """
        assert reason in (None, 'tp', 'sl', 'time','margin_call' , 'hedge'), \
            f"exit_reason must be one of None, 'tp', 'sl', 'time', 'margin_call','hedge', got {reason!r}"
...
class _Broker:
    def __init__(self, *, data, cash, spread, commission, margin,
        ...
        ############ our new attributes ############
        self.trade_count = 0  # Count of trades opened
        self.wining_trade_count = 0  # Count of winning trades
        self.total_hold_time = 0  # Total hold time in hours
        
        self.__exit_reasons_counts = {
            'tp': 0,
            'sl': 0,
            'time': 0,
            'margin_call': 0 ,
    ...
    def new_order(self,
        ...
        order = Order(self, size, limit, stop, sl, tp, trade, tag,hold_time)
    ...
    @property
    def exit_reasons_counts(self):
        """
        Count of exit reasons for closed trades.
        Returns a dictionary with keys 'tp', 'sl', 'time', etc. and their respective counts.
        """
        return self.__exit_reasons_counts
    
    # exit reasons setter
    @exit_reasons_counts.setter
    def exit_reasons_counts(self, reason: str):
        """
        Increment the count of exit reasons for closed trades.
        Possible values are 'tp', 'sl', 'time', 'margin_call', or 'hedge'.
        """
        self.__exit_reasons_counts[reason] += 1
    ...
    def next(self):
        ...
        if equity <= 0:
            ...
            for trade in self.trades:
                self._close_trade(trade, self._data.Close[-1], i , exit_reason='margin_call')
            self._cash = 0
            self._equity[i:] = 0
    ...
    def _process_orders(self):
        ...
        for order in list(self.orders):  # type: Order
            ...
            if trade is not None:
                ...
                time_diff_in_hours = (current_time - entry_time).total_seconds() / 3600
                if time_diff_in_hours >= trade.hold_time:
                    # If the hold time is reached, close the trade
                    price = data.Close[-1]
                    self._close_trade(order.parent_trade, price, self._i, exit_reason='time')
                    #continue
            ###########################
            # If order is contingent, it should be processed first
            ...
            if order.parent_trade:
                ...
                if trade in self.trades:
                    exit_reason ='tp' if order is trade._tp_order else 'sl'
            ...
            if not self._hedging:
                ...
                for trade in list(self.trades):
                    ...
                    if abs(need_size) >= abs(trade.size):
                        self._close_trade(trade, price, time_index ,exit_reason='hedge')
    ...
    def _reduce_trade(self, trade: Trade, price: float, size: float, time_index: int,exit_reason: Optional[str] = None):
    ...
    def _close_trade(self, trade: Trade, price: float, time_index: int,exit_reason: Optional[str] = None):
        ...
        closed_trade = trade._replace(exit_price=price, exit_bar=time_index, exit_reason=exit_reason)
        ...
        # record the trade counts as well if it was a winning trade
        self.trade_count += 1
        if closed_trade.pl > 0:
            self.wining_trade_count += 1
            
        # update the total active trading time
        self.total_hold_time += (closed_trade.exit_time - closed_trade.entry_time).total_seconds() / 3600
        
        # update the exit reasons counts
        self.exit_reasons_counts = exit_reason if exit_reason else 'time'

    def _open_trade(self, price: float, size: int,
                    sl: Optional[float], tp: Optional[float], time_index: int, tag , hold_time: Optional[float] = None):
        trade = Trade(self, size, price, time_index, tag , hold_time)
