...
from backtesting_v2 import Backtest, Strategy
from backtesting_v2._stats import compute_drawdown_duration_peaks
from backtesting_v2._util import _Array, _as_str, _Indicator, patch, try_
from backtesting_v2.lib import (
...
from backtesting_v2.test import BTCUSD, EURUSD, GOOG, SMA
...
class TestPlot(TestCase):
    ...
    def test_resample(self):
        ...
        bt = Backtest(GOOG, S)
        bt.run()
        import backtesting_v2._plotting
        with _tempfile() as f, \
                patch(backtesting_v2._plotting, '_MAX_CANDLES', 10), \
...
class TestLib(TestCase):
    ...
    def test_MultiBacktest(self):
        import backtesting_v2
        assert callable(getattr(backtesting_v2, 'Pool', None)), backtesting_v2.__dict__
        for start_method in mp.get_all_start_methods():
            with self.subTest(start_method=start_method), \
                    patch(backtesting_v2, 'Pool', mp.get_context(start_method).Pool):
